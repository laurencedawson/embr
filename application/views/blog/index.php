...
<?foreach($posts as $post){
  ...
  if(strlen($post['image'])){	
    ...
    echo '<p class="picture_info">'.($plus_one?'<span class="plus_one"><g:plusone size="small" count="false" href="'.$url.'"></g:plusone></span>':'').($post['source']?'<span class="source"><a href="'.$post['source'].'">'.$post['source'].'</a></span>':'').'<span class="author">Posted by '.$first_name.'</span><span class="info_separator"> on </span><span class="post_date"><span class="day">'.date("D",$time).'</span><span class="date">'.date("j",$time).'</span><span class="month">'.date("M",$time).'</span><span class="year">'.date("Y",$time).'</span></span></p>';
  }
